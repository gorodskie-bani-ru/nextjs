...
class WsProxy extends Component {
  ...
  SocketOnMessage(event) {
    ...
    try {
      ...
      const type =
        response.type ||
        (response.original_message ? response.original_message.type : '')
  ...
  log_connection_state() {
    this.props.proxyActions.SetConnectionState(
      (this.socket && this.socket.readyState == this.socket.OPEN) === true
    )
  }
  ...
  connect() {
    ...
    if (
      ...
      if (
        ...
        this.LastConnectAttempt > new Date().getTime() - 1000
      ) {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!nextProps.connection_allowed || nextProps.disconnection_requested) {
    ...
    if (
      nextProps.user.logoutRequested &&
      nextProps.user.logoutRequested !== this.props.user.logoutRequested
    ) {
      ...
      nextProps.user.get_own_data_success !=
        this.props.user.get_own_data_success &&
      nextProps.user.id > 0
    ) {
      this.props.proxyActions.SendMessage({
        type: 'joined',
    ...
    if (this.props.outbox.length) {
      if (!this.state.sending) {
        ...
        for (const i in this.props.outbox) {
          const message = this.props.outbox[i]
    ...
    if (
      nextProps.document.requested_url &&
      nextProps.document.requested_url != this.props.document.requested_url
    ) {
