...
export default class UsersPage extends Page {
  ...
  loadData() {
    const { delegatesOnly, myOnly } = this.state
  ...
  async loadServerData(provider, options = {}) {
    ...
    const { page, delegatesOnly, myOnly } = options
  ...
  updateUser(user, data) {
    const { updateItem, UsersStore } = this.context
  ...
  saveItem(user) {
    const { saveItem, UsersStore } = this.context
  ...
  renderUsers() {
    ...
    const { user: currentUser, updateItem } = this.context

    const { usersList, delegatesOnly, myOnly, compredOpen } = this.state

    // if(!usersList){
    // 	return null;
    // }

    const { page, limit, total, object: users } = usersList || {}
    ...
    users &&
      users.map((user) => {
        ...
        const columns = [
          ...
          <TableCell key="comments">
            {(comments && comments.length) || ''}
          </TableCell>,

          <TableCell key="email">{email || 'Нет прав на просмотр'}</TableCell>,
        ]
        ...
        if (hasCRMPerm) {
          ...
          columns.push(<TableCell key="manager">{createdby || ''}</TableCell>)
          ...
          columns.push(
            <TableCell key="offerDate">
              <TextField
                type="date"
                name="offer_date"
                value={
                  (offer_date &&
                    moment(offer_date * 1000).format('YYYY-MM-DD')) ||
                  ''
                }
          ...
          columns.push(
            <TableCell key="contract">
              <TextField
                ...
                value={
                  (contract_date &&
                    moment(contract_date * 1000).format('YYYY-MM-DD')) ||
                  ''
                }
                onChange={(event) => this.onUpdateField(event, user)}
              />
        ...
        rows.push(<TableRow key={id}>{columns}</TableRow>)
      })
    ...
    const columns = [
      <TableCell key="user">Пользователь</TableCell>,

      <TableCell key="createdon">Дата регистрации</TableCell>,

      <TableCell key="comment">Комментарии</TableCell>,

      <TableCell key="email">Емейл</TableCell>,
    ]

    if (hasCRMPerm) {
      columns.unshift(<TableCell key="actions">Действия</TableCell>)

      columns.push(<TableCell key="manager">Менеджер</TableCell>)

      columns.push(<TableCell key="isActive">Активен</TableCell>)

      columns.push(<TableCell key="delegate">Представитель</TableCell>)

      columns.push(<TableCell key="offer_date">Дата предложения</TableCell>)

      columns.push(<TableCell key="contract">Сделка</TableCell>)

      columns.push(<TableCell key="offer">Предложение</TableCell>)
    }
        ...
        {hasCRMPerm ? (
          ...
          >
            <Grid container align="center" gutter={0}>
              ...
              <Checkbox checked={myOnly} onChange={this.onMyOnlyChange} />{' '}
              Только мои
              <Button
                onClick={(event) => {
                  this.setState({
