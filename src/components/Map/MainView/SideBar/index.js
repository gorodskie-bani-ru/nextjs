...
export default class SideBar extends Component {
  ...
  render() {
    ...
    const { sideBarFullHeight } = this.state
    ...
    icons.push(
      <div key={icons.length} className="icon-block">
        <Button>Клубы, школы, секции</Button>
      </div>
    )

    icons.push(
      <div key={icons.length} className="icon-block">
        <Button>Спортивные объекты</Button>
      </div>
    )

    icons.push(
      <div key={icons.length} className="icon-block">
        <Button>Спорт-бары</Button>
      </div>
    )

    icons.push(
      <div key={icons.length} className="icon-block">
        <Button>Спорт-shop</Button>
      </div>
    )
      ...
      >
        <div className="SideBar--local">
          <div className="SideBar--panels">
            ...
            {icons ? (
              <div className="SideBar--block icons">
                <div
                  style={{
                    position: 'absolute',
                    left: 0,
