...
class LayoutComponent extends Component {
  ...
  componentWillReceiveProps(props: Record<string, any>): void {
  ...
  updateValue: Function = (event: Record<string, any>): void => {
    this.setState({
  ...
  onImageDrop: Function = (event: Record<string, any>): void => {
  ...
  onDragEnter: Function = (event: Record<string, any>): void => {
  ...
  selectImage: Function = (event: Record<string, any>): void => {
    if (event.target.files && event.target.files.length > 0) {
      this.uploadImage(event.target.files[0])
    }
  }

  uploadImage: Function = (file: Record<string, any>): void => {
  ...
  stopPropagation: Function = (event: Record<string, any>): void => {
    if (!this.fileUpload) {
      event.preventDefault()
      event.stopPropagation()
    } else {
  ...
  renderAddImageModal(): Record<string, any> {
    const {
      imgSrc,
      uploadHighlighted,
      showImageLoading,
