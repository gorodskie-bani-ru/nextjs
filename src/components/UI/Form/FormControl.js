...
// import { isMuiComponent } from 'material-ui/utils/reactHelpers';

const isMuiComponent = function (element: any, muiName: string) {
  return isValidElement(element) && element.type.muiName === muiName
}

export const styleSheet = createStyleSheet(
  'MuiFormControlModified',
  (theme) => ({
    root: {
      display: 'inline-flex',
...
class FormControl extends Component<DefaultProps, AllProps, State> {
  ...
  getChildContext() {
    ...
    const { dirty: propsDirty } = this.props

    // const dirty = true;

    return {
      muiFormControl: {
        dirty: propsDirty !== undefined ? propsDirty : dirty,
        disabled,
        error,
        focused,
        margin,
