...
import { YaAutoComplete } from 'material-ui-components/src/YandexAutoComplete'
...
class YandexAutoCompleteInner extends YaAutoComplete {
  ...
  async loadData() {
    const { includeSiteData } = this.props

    const { localQuery, remoteQuery } = this.context
    ...
    if (searchText && searchText.length > 2) {
      await ymaps.geocode(searchText).then((res) => {
        ...
        await localQuery({
          ...
          .then((r) => {
            ...
            if (search && search.length) {
              search.map((n) => {
                const { id, name, coords, imageFormats } = n

                if (!coords) {
                  return
                }

                const { thumb: image } = imageFormats || {}
...
class YandexAutoComplete extends Component {
  ...
  render() {
    const props = this.cleanupProps(this.props)

    return (
      <YMaps
        children={function (ymaps) {
          return (
            <YandexAutoCompleteInner
              ymaps={ymaps}
              // map={map}
              // google={google}
              // onChange={event => {
