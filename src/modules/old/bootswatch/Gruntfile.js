module.exports = function (grunt) {
  ...
  const configBridge = grunt.file.readJSON('./bower_components/bootstrap/grunt/configBridge.json', { encoding: 'utf8' });
  ...
  grunt.registerTask('build', 'build a regular theme', function(theme, compress) {
    ...
    const isValidTheme = grunt.file.exists(theme, 'variables.less') && grunt.file.exists(theme, 'bootswatch.less');
...
grunt.registerTask('build_scss', 'build a regular theme from scss', function(theme, compress) {
    ...
    const isValidTheme = grunt.file.exists(theme, '_variables.scss') && grunt.file.exists(theme, '_bootswatch.scss');
  ...
  grunt.registerTask('compress', 'compress a generic css', function(fileSrc, fileDst) {
    const files = {}; files[fileDst] = fileSrc;
  ...
  grunt.event.on('watch', function(action, filepath) {
    ...
    const theme = path.dirname(filepath);
  ...
  grunt.registerTask('convert_less', 'Convert less to scss using regular expression', function () {
    ...
    grunt.file.expand(convertBaseDir + '*/*.less').forEach(function (lessFile) {
      if (lessFile !=="global/build.less"){
        ...
        const baseDirRegex = new RegExp("^" + convertBaseDir, "g");
        const sassFile = lessFile.replace(baseDirRegex, '').replace(/\.less$/, '.scss').replace(/(bootswatch|variables)/, '_$1');
